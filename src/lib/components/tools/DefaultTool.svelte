...
>
	<div class="space-y-2">
		{#if toolInvocation.args && Object.keys(toolInvocation.args).length > 0}
			<div>
				<h5 class="mb-1 font-medium text-gray-700 dark:text-gray-300">Input:</h5>
				<pre class="overflow-x-auto rounded bg-gray-100 p-2 text-xs dark:bg-gray-900"><code
						>{JSON.stringify(toolInvocation.args, null, 2)}</code
					></pre>
			</div>
		{/if}

		<div>
			<h5 class="mb-1 font-medium text-gray-700 dark:text-gray-300">Output:</h5>
			<pre class="overflow-x-auto rounded bg-gray-100 p-2 text-xs dark:bg-gray-900"><code
					>{displayData()}</code
				></pre>
		</div>
