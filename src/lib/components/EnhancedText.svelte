<script lang="ts">
	...
	// Parse the text for custom formatting first
		...
		try {
			...
			return customSegments.map((segment) => {
				if (segment.isFormatted) {
					...
					// Apply markdown to unformatted text
			...
			// Fallback to just markdown if custom formatting fails
			return [
				{
					text: enableMarkdown ? renderMarkdown(text) : text,
					isFormatted: false,
